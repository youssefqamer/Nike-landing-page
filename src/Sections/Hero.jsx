...
const Hero = () => {
  ...
  return (
    ...
    <h1 className=" mt-10 font-palanquin text-8xl max-sm:text-[72px] max-md:leading[82] font-bold">
      ...
      <div className="relative flex flex-1 justify-center items-center xl:min-h-screen max-xl:py-40 bg-primary bg-hero bg-cover">
        ...
        <div className="flex sm:gap-6 gap-4 absolute -bottom-[5%] sm:left-[10%] max-sm:px-6">
          {shoes.map((shoe,index)=>(
            <div key={index}>
              <ShoeCard imgUrl={shoe} chnageBigShoeImg={(currentImg)=>{setBigShoeImg(currentImg)}} bigShoeImg={bigShoeImg}/>
            </div>
          ))}
        </div>
      </div>
